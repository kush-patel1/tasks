...
export function stringsToIntegers(numbers: string[]): number[] {
    const retArray = numbers.map((num: string) =>
        isNaN(parseInt(num, 10)) ? 0 : parseInt(num, 10),
    );
    return retArray;
    /*Had to use the help of ChatGPT for this one. If parseInt returns NaN, then '?' replaces it with 0.*/
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const withoutQM = messages.filter(
        (message: string) => !message.endsWith("?"),
    );
    const shoutedMessages = withoutQM.map((message: string) =>
...
export function countShortWords(words: string[]): number {
    const count = words.filter((word: string) => word.length < 4).length;
    return count;
}
