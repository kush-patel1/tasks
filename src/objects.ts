...
export function toShortForm(question: Question): string {
    let shortName: string = question.name.substring(0, 10);
...
export function toMarkdown(question: Question): string {
    let retString: string = "# " + question.name + "\n" + question.body + "\n";
    if (question.type === "multiple_choice_question") {
    ...
    /* Used stack overflow to discover .forEach */
}
...
export function publishQuestion(question: Question): Question {
    const published = { ...question, published: !question.published };
    return published;
}
